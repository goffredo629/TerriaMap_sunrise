...
export default class TableAutomaticStylesStratum extends LoadableStratum(TableTraits) {
    ...
    get activeStyle() {
        if (this.catalogItem.styles && this.catalogItem.styles.length > 0) {
            ...
            const firstStyleOfEachType = types.map((columnType) => this.catalogItem.styles
                .filter((style) => !style.hidden)
                .find((s) => this.catalogItem.findColumnByName(s.color?.colorColumn)
                ?.type === columnType)?.id);
    ...
    get showDisableTimeOption() {
        ...
        for (let i = 0; i < this.catalogItem.activeTableStyle.rowGroups.length; i++) {
            ...
            const dates = rowIds
                .map((rowId) => this.catalogItem.activeTableStyle.timeColumn?.valuesAsDates.values[rowId]?.getTime())
                .filter(isDefined);
