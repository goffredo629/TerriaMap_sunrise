...
let SharePanel = class SharePanel extends Component {
    ...
    renderContent() {
        ...
        if (this.props.catalogShare) {
            return (_jsxs(Box, { fullWidth: true, column: true, paddedRatio: 3, children: [_jsx(Text, { medium: true, children: t("clipboard.shareURL") }), _jsx(Spacing, { bottom: 1 }), _jsx(ShareUrl, { terria: terria, viewState: viewState, includeStories: true, shouldShorten: shouldShorten(terria) })] }));
        }
        else if (this.props.storyShare) {
            return (_jsxs(Box, { fullWidth: true, column: true, paddedRatio: 3, children: [_jsx(Text, { medium: true, children: t("share.shareStoryLink") }), _jsx(Spacing, { bottom: 1 }), _jsx(ShareUrl, { terria: terria, viewState: viewState, includeStories: true, shouldShorten: shouldShorten(terria) })] }));
        }
        else {
...
export function shouldShorten(terria) {
    return (stringToBool(terria.getLocalProperty("shortenShareUrls")) ??
        !!canShorten(terria));
}
