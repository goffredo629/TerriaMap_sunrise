...
function buildBaseShareUrl(terria, hashParams) {
    ...
    const fragmentsToShare = new URL(document.URL).hash
        .split(/[#&]/)
        .filter((elem) => elem !== "" && !elem.includes("share=") && !elem.includes("start="));
...
export async function buildShortShareLink(terria, viewState, options = { includeStories: true }) {
    ...
    const token = await terria.shareDataService?.getShareToken(getShareData(terria, viewState, options));
...
function addModelStratum(terria, model, stratumId, force, initSource) {
    ...
    if (Array.isArray(members)) {
        models[id].members = uniq(models[id].members?.filter((member) => typeof member === "string" ? isShareable(terria)(member) : false));
    }
