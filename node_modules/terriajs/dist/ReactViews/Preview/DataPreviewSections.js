...
let DataPreviewSections = class DataPreviewSections extends Component {
    ...
    sortInfoSections(items) {
        ...
        return items.filter((item) => isDefined(item.content ?? item.contentAsObject) &&
    ...
    render() {
        ...
        return (_jsx("div", { children: this.sortInfoSections(items).map((item, i) => (_jsx(Box, { paddedVertically: true, displayInlineBlock: true, fullWidth: true, children: _jsx(Collapsible, { light: false, title: item.name, isOpen: item.show, onToggle: (show) => this.clickInfoSection.bind(this, item.name, show)(), bodyTextProps: { medium: true }, children: item.content?.length > 0
                        ? renderSection(item)
