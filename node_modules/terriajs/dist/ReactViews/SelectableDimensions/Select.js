...
export const SelectableDimensionEnum = observer(({ dim }) => {
    ...
    const undefinedOption = {
        value: undefined,
        label: dim.undefinedLabel ?? i18next.t("selectableDimensions.undefinedLabel")
    };
    ...
    return dim.allowCustomInput ? (_jsx(ReactSelectCreatable, { css: `
      ...
      `, styles: {
            ...
            runInAction(() => dim.setDimensionValue(CommonStrata.user, evt?.value ?? ""));
...
export const SelectableDimensionEnumMulti = observer(({ dim }) => {
    ...
    if (!options)
        return null;
    const selectedOptions = options.filter((option) => dim.selectedIds?.some((id) => option.value === id));
    return (_jsx(ReactSelect, { css: `
        color: ${theme.dark};
        border: none;
      `, options: options, value: selectedOptions, onChange: (evt) => {
            runInAction(() => dim.setDimensionValue(CommonStrata.user, evt?.map((selected) => selected.value).filter(isDefined) ?? []));
        }, isClearable: dim.allowUndefined, formatOptionLabel: dim.optionRenderer, theme: (selectTheme) => ({
            ...selectTheme,
            colors: {
                ...selectTheme.colors,
