...
class WebCoverageServiceCapabilitiesStratum extends LoadableStratum(ExportWebCoverageServiceTraits) {
    ...
    static async load(catalogItem) {
        ...
        const coverages = json.Contents?.CoverageSummary ?? [];
        const formats = json.ServiceMetadata?.formatSupported ?? [];
        const crs = json.ServiceMetadata?.Extension?.CrsMetadata?.crsSupported ?? [];
...
class WebCoverageServiceDescribeCoverageStratum extends LoadableStratum(ExportWebCoverageServiceTraits) {
    ...
    static async load(catalogItem) {
        ...
        const nativeFormat = json.CoverageDescription?.ServiceParameters?.nativeFormat;
        // Try get native CRS from domainSet and then boundedBy
        const nativeCrs = json.CoverageDescription?.domainSet?.Grid?.srsName ??
...
function ExportWebCoverageServiceMixin(Base) {
    class ExportWebCoverageServiceMixin extends ExportableMixin(Base) {
        _wcsCapabilitiesLoader = new AsyncLoader(this.loadWcsCapabilities.bind(this));
        _wcsDescribeCoverageLoader = new AsyncLoader(this.loadWcsDescribeCoverage.bind(this));
        constructor(...args) {
            super(...args);
            makeObservable(this);
        }
            ...
            try {
                ...
                const query = {
                    ...
                    subset: [
                        ...
                        ...filterOutUndefined((this.linkedWcsParameters.subsets ?? []).map((subset) => subset.key && subset.value
                            ? `${subset.key}(${
                ...
                Object.assign(query, (this.linkedWcsParameters.additionalParameters ?? []).reduce((q, current) => {
                ...
                if (error instanceof RequestErrorEvent &&
                    ...
                    try {
                        ...
                        if (xml.documentElement.localName === "ServiceExceptionReport" ||
                            ...
                            const message = xml.getElementsByTagName("ServiceException")?.[0]?.innerHTML ??
