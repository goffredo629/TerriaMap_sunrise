...
function TableMixin(Base) {
    class TableMixin extends ExportableMixin(DiscretelyTimeVaryingMixin(MappableMixin(CatalogMemberMixin(Base)))) {
        ...
        get mapItems() {
            ...
            if (this.dataColumnMajor?.length === 0 ||
                !this.activeTableStyle.ready ||
                this.isLoadingMapItems)
                return [];
            const numRegions = this.activeTableStyle.regionColumn?.valuesAsRegions?.uniqueRegionIds
                ?.length ?? 0;
        ...
        matchRegionProvider(regionType) {
            if (!isDefined(regionType))
                return;
            const matchingRegionProviders = this.regionProviderLists?.map((regionProviderList) => regionProviderList?.getRegionDetails([regionType], undefined, undefined));
            ...
            return matchingRegionProviders?.find((match) => match && match.length > 0)?.[0].regionProvider;
        }
        /**
        ...
        get regionProviderDimensions() {
            const allRegionProviders = flatten(this.regionProviderLists?.map((list) => list.regionProviders) ?? []);
            ...
            return {
                ...
                setDimensionValue: (stratumId, regionType) => {
                    let columnTraits = this.columns?.find((column) => column.name === this.activeTableStyle.regionColumn?.name);
        ...
        createLongitudeLatitudeDataSource = createTransformer((style) => {
            if (!style.isPoints()) {
                return undefined;
            }
        ...
        createRegionMappedImageryProvider = createTransformer((input) => createRegionMappedImageryProvider(input.style, input.currentTime));
        getTableColumn = createTransformer((index) => {
            return new TableColumn(this, index);
        });
        getTableStyle = createTransformer((index) => {
