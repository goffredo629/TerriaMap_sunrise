...
function GroupMixin(Base) {
    class _GroupMixin extends Base {
            ...
            membersByName.forEach((groups, name) => {
                if (groups.length > 1) {
                    ...
                    if (GroupMixin.isMixedInto(mergedGroup) &&
                        ...
                        this.setTrait(CommonStrata.override, "excludeMembers", uniq([...(this.excludeMembers ?? []), ...groupIdsToMerge]));
                    }
                }
            });
        }
        ...
        addMembersFromJson(stratumId, members) {
            ...
            newMemberIds
                .ignoreError()
                ?.map((memberId) => this.terria.getModelById(BaseModel, memberId))
                .forEach((member) => {
...
export function applyItemProperties(model, target) {
    runInAction(() => {
        ...
        setItemPropertyTraits(target, model.itemPropertiesByType.find((itemProps) => itemProps.type && itemProps.type === target.type)?.itemProperties);
