...
export default class Leaflet extends GlobeOrMap {
    ...
    _eventHelper;
    _selectionIndicator;
    _stopRequestAnimationFrame = false;
    _cesiumReqAnimFrameId;
    _pickedFeatures = undefined;
    _pauseMapInteractionCount = 0;
    /* Disposers */
    _disposeWorkbenchMapItemsSubscription;
    _disposeDisableInteractionSubscription;
    ...
    async _featurePicked(entity, event) {
        ...
        if (FeatureInfoUrlTemplateMixin.isMixedInto(catalogItem) &&
            ...
            const result = await catalogItem.getFeaturesFromPickResult.bind(catalogItem)(undefined, entity, (this._pickedFeatures?.features.length || 0) < catalogItem.maxRequests);
    ...
    _pickFeatures(latlng, tileCoordinates, existingFeatures, ignoreSplitter = false) {
        ...
        const imageryFeaturePromises = imageryLayers.map(async (imageryLayer) => {
            ...
            const coords = tileCoordinates?.[imageryLayerUrl] ??
                (await imageryLayer.getFeaturePickingCoords(this.map, longRadians, latRadians));
