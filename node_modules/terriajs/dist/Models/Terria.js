    ...
    async _applyInitData({ initData, replaceStratum = false, canUnsetFeaturePickingState = false }) {
        ...
        if (isJsonObject(initData.settings)) {
            ...
            if (isJsonString(initData.settings.baseMapId)) {
                baseMapPromise = this.mainViewer.setBaseMap(this.baseMapsModel.baseMapItems.find((item) => item.item.uniqueId === initData.settings.baseMapId)?.item);
            }
        ...
        newItems.forEach((item) => {
            ...
            this.analytics?.logEvent(Category.dataSource, DataSourceAction.addFromShareOrInit, getPath(item));
        });
    ...
    async loadPickedFeatures(pickedFeatures) {
        ...
        runInAction(() => {
            ...
            if (isJsonObject(current) && typeof current.hash === "number") {
                const selectedFeature = (featureIndex[current.hash] || []).find((feature) => feature.name === current.name) ?? featureIndex[current.hash]?.[0];
                if (selectedFeature) {
...
__decorate([
    action
], Terria.prototype, "_applyInitData", null);
__decorate([
    action
