...
export class OpenDataSoftCatalogStratum extends LoadableStratum(OpenDataSoftCatalogGroupTraits) {
    ...
    static async load(catalogGroup) {
        ...
        if (catalogGroup.facetFilters &&
            ...
            facets = (await client.get(fromCatalog().facets())).facets?.filter((f) => isValidFacet(f));
        }
        ...
        return new OpenDataSoftCatalogStratum(catalogGroup, undefined, facets ?? [], datasets ?? []);
    }
    ...
    createGroupFromFacet(facet) {
        ...
        groupModel.setTrait(CommonStrata.definition, "name", `${facet.name}${facet.count ? ` (${facet.count ?? 0})` : ""}`);
    ...
    createMemberFromDataset(dataset) {
        ...
        itemModel.setTrait(CommonStrata.definition, "name", dataset.metas?.default?.title ?? dataset.dataset_id);
        itemModel.setTrait(CommonStrata.definition, "description", dataset.metas?.default?.description ?? undefined);
...
export default class OpenDataSoftCatalogGroup extends UrlMixin(GroupMixin(CatalogMemberMixin(CreateModel(OpenDataSoftCatalogGroupTraits)))) {
    static type = "opendatasoft-group";
    get type() {
...
export function isValidFacet(facet) {
    return (isDefined(facet) &&
        ...
        (facet.facets ?? []).reduce((valid, current) => valid && isValidFacet(current), true));
}
