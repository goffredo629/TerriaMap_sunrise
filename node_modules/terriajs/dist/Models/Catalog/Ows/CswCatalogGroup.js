...
class CswStratum extends LoadableStratum(CswCatalogGroupTraits) {
    ...
    static async load(catalogGroup) {
        ...
        if (!catalogGroup.flatten &&
            ...
            const listOfValues = flatten(toArray(domainResponse.DomainValues)?.map((d) => toArray(d?.ListOfValues?.Value))).filter((v) => typeof v === "string");
        ...
        while (paging) {
            ...
            const postData = (catalogGroup.getRecordsTemplate ?? defaultGetRecordsTemplate).replace("{startPosition}", startPosition);
            ...
            const nextRecord = typeof json?.SearchResults?.nextRecord === "string"
                ? parseInt(json?.SearchResults?.nextRecord ?? "0", 10)
                : json?.SearchResults?.nextRecord;
            const numberOfRecordsMatched = typeof json?.SearchResults?.numberOfRecordsMatched === "string"
    ...
    createMetadataGroup(parentId, metadataGroup) {
        ...
        model.setTrait(CommonStrata.definition, "members", filterOutUndefined([
            ...
            ...metadataGroup.records.map((record) => this.createRecord(layerId, record)?.uniqueId)
        ]));
        return model;
    }
    createRecord(parentId, record) {
        ...
        for (let m = 0; m < uris.length; m++) {
            ...
            if (!uri)
                return;
            const resourceIndex = filteredResourceFormats.findIndex((f) => (uri.protocol ?? uri.scheme)?.match(f.regex));
        ...
        if (urlIndex !== -1) {
            ...
            // Replace the stratum inherited from the parent group.
            model.strata.delete(CommonStrata.definition);
            model.setTrait(CommonStrata.definition, "name", record.title ?? record.identifier);
            const uri = acceptableUris[urlIndex];
            model.setTrait(CommonStrata.definition, "url", uri.toString());
            if (record.abstract) {
                model.setTrait(CommonStrata.definition, "description", toArray(record.abstract)?.join("\n\n"));
            }
            else if (record.description) {
                model.setTrait(CommonStrata.definition, "description", toArray(record.description)?.join("\n\n"));
            }
            const infoSections = [];
