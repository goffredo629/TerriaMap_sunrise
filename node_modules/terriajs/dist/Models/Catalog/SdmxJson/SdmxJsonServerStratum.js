...
export class SdmxServerStratum extends LoadableStratum(SdmxCatalogGroupTraits) {
    ...
    static async load(catalogGroup) {
        ...
        const agencySchemes = (await loadSdmxJsonStructure(proxyCatalogItemUrl(catalogGroup, `${catalogGroup.url}/agencyscheme/`), true))?.data?.agencySchemes;
        ...
        if (!isDefined(dataflows)) {
            dataflows = (await loadSdmxJsonStructure(proxyCatalogItemUrl(catalogGroup, `${catalogGroup.url}/dataflow/`), true))?.data?.dataflows;
            if (!isDefined(dataflows)) {
                ...
                categoryIds.forEach((categoryId) => {
                    ...
                    if (!isDefined(categoryParentNode.members[categoryId])) {
                        ...
                        if (categoryParentNode.type === "categoryScheme") {
                            ...
                            category = this.getCategoryFromCategories(categoryParentNode.item?.categories, categoryId);
                        }
    ...
    get members() {
        ...
        const findRootGroup = (node) => {
            const children = isDefined(node?.members)
        ...
        if (rootTreeNodes.length === 1 && isDefined(rootTreeNodes[0])) {
            rootTreeNodes = Object.values(findRootGroup(rootTreeNodes[0]).members ?? this.dataflowTree);
        }
    ...
    getCategoryFromCatagoryScheme(categoryScheme, id) {
        ...
        const resolvedCategoryScheme = typeof categoryScheme === "string"
            ...
            : categoryScheme;
        return this.getCategoryFromCategories(resolvedCategoryScheme?.categories, id);
    }
    ...
    getAgency(id) {
        if (!isDefined(id))
            return;
        const agencies = this.sdmxServer.agencySchemes?.map((agencyScheme) => agencyScheme.agencies);
