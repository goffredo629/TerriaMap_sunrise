...
export class SdmxJsonDataflowStratum extends LoadableStratum(SdmxCatalogItemTraits) {
     ...
     */
    get metadataUrls() {
        return filterOutUndefined(this.sdmxJsonDataflow?.dataflow.annotations
    ...
    get sdmxAttributes() {
        return (this.sdmxJsonDataflow.dataStructure.dataStructureComponents?.attributeList
            ?.attributes ?? []);
    }
    get sdmxDimensions() {
        return (this.sdmxJsonDataflow.dataStructure.dataStructureComponents?.dimensionList
            ?.dimensions ?? []);
    }
    ...
    get shortReportSections() {
        if (this.catalogItem.mapItems.length !== 0 || this.catalogItem.isLoading)
            return;
        const primaryCol = this.catalogItem.tableColumns.find((col) => col.name === this.primaryMeasureColumn?.name);
        if (primaryCol?.valuesAsNumbers.values.length === 1 &&
            typeof primaryCol?.valuesAsNumbers.values[0] === "number") {
            return [
                createStratumInstance(ShortReportTraits, {
                    name: this.chartTitle,
                    content: primaryCol?.valuesAsNumbers.values[0].toLocaleString(undefined, primaryCol.traits.format)
                })
            ];
        }
    }
    ...
    getMergedModelOverride(dim) {
        ...
        const codelistOverride = this.catalogItem.modelOverrides.find((codelist) => codelist.id === dim.localRepresentation?.enumeration);
    ...
    get dimensions() {
        ...
        const constraints = this.sdmxJsonDataflow.contentConstraints?.filter((c) => c.type === "Actual");
        return (this.sdmxDimensions
            ...
            const codelist = this.getCodelistByUrn(dim.localRepresentation?.enumeration);
            ...
            const allowedOptionIds = Array.isArray(constraints)
                ? constraints.reduce((keys, constraint) => {
                    constraint.cubeRegions?.forEach((cubeRegion) => cubeRegion.keyValues
                        ?.filter((kv) => kv.id === dim.id)
                        ?.forEach((regionKey) => regionKey.values?.forEach((value) => keys.add(value))));
                    return keys;
                }, new Set())
                : new Set();
            // Get codes by merging allowedOptionIds with codelist
            const filteredCodesList = (allowedOptionIds.size > 0
            ...
            let selectedId = modelOverride?.allowUndefined
                ? undefined
                : options[0]?.id;
    ...
    get unitMeasure() {
        ...
        const unitMeasure = filterOutUndefined(this.catalogItem.modelOverrides
            ?.filter((override) => override.type === "unit-measure" && override.id)
            .map((override) => {
            // Find dimension/attribute id with concept or codelist override
            const dimOrAttr = this.getAttributionWithConceptOrCodelist(override.id) ??
            ...
            if (column?.uniqueValues.values.length === 1) {
                ...
                const codelist = this.getCodelistByUrn(dimOrAttr?.localRepresentation?.enumeration);
        ...
        const frequency = frequencyDim?.options.find((o) => o.id === frequencyDim.selectedId)
    ...
    get primaryMeasureColumn() {
        if (!this.sdmxPrimaryMeasure)
            return;
        const primaryMeasureConcept = this.getConceptByUrn(this.sdmxPrimaryMeasure?.conceptIdentity);
        ...
        const unitMultiplier = filterOutUndefined(this.catalogItem.modelOverrides
    ...
    get dimensionColumns() {
        ...
        return (this.sdmxDimensions
            ...
            regionType = this.catalogItem.matchRegionProvider(modelOverride?.regionType)?.regionType;
            ...
            if (modelOverride?.type === "region") {
                ...
                regionType =
                    this.catalogItem.matchRegionProvider(this.getDimensionsWithOverrideType("region-type").find((d) => isDefined(d.selectedId))?.selectedId)?.regionType ?? regionType;
            }
            ...
            if (modelOverride?.type === "region") {
                const replacement = modelOverride?.regionTypeReplacements?.find((r) => r.find === regionType)?.replace;
     ...
     */
    get timeColumns() {
        return (this.sdmxTimeDimensions.map((dim) => {
            const concept = this.getConceptByUrn(dim.conceptIdentity);
            return createStratumInstance(TableColumnTraits, {
                name: dim.id,
    ...
    get attributeColumns() {
        return (this.sdmxAttributes.map((attr) => {
            return createStratumInstance(TableColumnTraits, {
                name: attr.id,
                type: "hidden"
            });
    ...
    get resolvedRegionColumn() {
        return this.catalogItem.tableColumns.find((tableCol) => tableCol.name ===
    ...
    get chartTitle() {
        if (this.disableRegion) {
            ...
            if (regionValues && regionValues.length === 1) {
                ...
                const regionDimensionId = this.getDimensionsWithOverrideType("region")[0]?.id;
                ...
                if (regionDimension) {
                    ...
                    const codelist = this.getCodelistByUrn(regionDimension.localRepresentation?.enumeration);
                    const regionName = codelist?.codes?.find((c) => c.id === regionValues[0])?.name ??
                        regionValues[0];
    ...
    get featureInfoTemplate() {
        ...
        template += filterEnums(this.catalogItem.sdmxSelectableDimensions)
            ...
            .join("");
        const primaryMeasureName = this.unitMeasure ??
            this.primaryMeasureColumn?.title ??
            this.primaryMeasureColumn?.name ??
            "Value";
        template +=
            row("", "") +
                row(primaryMeasureName, `{{#terria.formatNumber}}{useGrouping: true}{{${this.primaryMeasureColumn?.name}}}{{/terria.formatNumber}}`);
    ...
    getDimensionsWithOverrideType(type) {
        return filterOutUndefined(this.catalogItem.modelOverrides
            ?.filter((override) => override.type === type && override.id)
            .map((override) => {
            // Find dimension id with concept or codelist
            return this.catalogItem.dimensions?.find((d) => d.id === this.getDimensionWithConceptOrCodelist(override.id)?.id);
        }));
    }
    getDimensionWithConceptOrCodelist(id) {
        return this.sdmxDimensions.find((dim) => dim.conceptIdentity === id ||
            dim.localRepresentation?.enumeration === id);
    }
    getAttributionWithConceptOrCodelist(id) {
        return this.sdmxAttributes.find((attr) => attr.conceptIdentity === id ||
