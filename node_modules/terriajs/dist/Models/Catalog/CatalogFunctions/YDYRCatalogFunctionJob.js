...
export default class YDYRCatalogFunctionJob extends CatalogFunctionJobMixin(CreateModel(YDYRCatalogFunctionJobTraits)) {
    ...
    async _invoke() {
        ...
        jobDetails?.setTrait(CommonStrata.user, "content", `${dataColumnName}: "${DATASETS.find((d) => d.geographyName === regionColumn?.regionType?.regionType)?.title}" to "${outputGeographyName}"`);
        const data = {
        ...
        const params = {
            ...
            dst_geom: DATASETS.find((d) => d.title === outputGeographyName)
                ?.geographyName,
            src_geom: tableCatalogItem?.activeTableStyle.regionColumn?.regionType?.regionType,
