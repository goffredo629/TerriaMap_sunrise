...
- A data column containing the values you want to convert (eg number of households by postcode)`,
    ...
    get regionColumn() {
        ...
        const possibleValues = this.selectedTableCatalogMember?.tableColumns
            // Filter region columns which use supported regions
            .filter((col) => col.type === TableColumnType.region &&
            isDefined(DATASETS.find((d) => d.dataCol === col.regionType?.regionProp)))
            .map((col) => {
            return { id: col.name };
        }) || [];
        return new EnumerationParameter(this, {
