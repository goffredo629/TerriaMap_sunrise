...
export default class GtfsCatalogItem extends UrlMixin(AutoRefreshingMixin(MappableMixin(CatalogMemberMixin(CreateModel(GtfsCatalogItemTraits))))) {
     ...
     */
    _dataSource = new DataSource("billboard");
    static FEATURE_INFO_TEMPLATE_FIELDS = [
        "route_short_name",
    ...
    gtfsFeedEntities = [];
    convertManyFeedEntitiesToBillboardData = createTransformer((feedEntities) => {
        ...
        for (const data of vehicleData) {
            ...
            if (!entity.model) {
                if (this._coloredModels) {
                    const gtfsEntity = data.featureInfo?.get("entity");
    ...
    get _coloredModels() {
        ...
        return colorGroups.map(({ color }) => {
            ...
            coloredModel.color = new ConstantProperty(Color.fromCssColorString(color ?? "white"));
