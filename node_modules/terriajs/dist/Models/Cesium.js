...
export default class Cesium extends GlobeOrMap {
    ...
    scene;
    pauser;
    canShowSplitter = true;
    _eventHelper;
    _3dTilesetEventListeners = new Map(); // eventListener reference storage
    _pauseMapInteractionCount = 0;
    _lastZoomTarget;
    // Lightbox and on screen attributions from CreditDisplay
    cesiumDataAttributions = observable([]);
    // Public because this is accessed from BottomLeftBar.tsx
    cesiumScreenDataAttributions = observable([]);
    ...
    /* Disposers */
    _selectionIndicator;
    _disposeSelectedFeatureSubscription;
    _disposeWorkbenchMapItemsSubscription;
    _disposeTerrainReaction;
    _disposeSplitterReaction;
    _disposeResolutionReaction;
    _createImageryLayer = computedFn((ip, clippingRectangle) => {
        return new ImageryLayer(ip, {
            rectangle: clippingRectangle
        });
    });
    _terrainMessageViewed = false;
    /**
     ...
     */
    terriaPrimitives = new PrimitiveCollection();
    ...
    updateCredits(container) {
        ...
        if (creditsElement) {
            if (logoContainer)
                creditsElement?.removeChild(logoContainer);
            if (expandLink)
                creditsElement?.removeChild(expandLink);
        }
    ...
    observeModelLayer() {
        ...
        const disposeDataSourcesSyncReaction = reaction(() => this.availableDataSources, () => {
    ...
    syncImagery() {
        ...
        this._allMappables.forEach((m) => {
            if (ImageryParts.is(m.mapItem)) {
            ...
            else if (Cesium3dTilesMixin.isMixedInto(m.item) &&
                ...
                syncImageryCollection(tileset.imageryLayers, m.item.drapeImagery ? topImageryLayers : [], false);
            }
        });
    ...
    subscribeTileLoadEvents(primitive) {
        ...
        this._3dTilesetEventListeners.set(primitive, function unregisterListeners() {
            startingListener();
            finishedListener();
        });
    }
    unsubscribeTileLoadEvents(primitive) {
    ...
    pickImageryLayerFeatures(pickPosition, providerCoords) {
        ...
        for (let i = this.scene.imageryLayers.length - 1; i >= 0; i--) {
            ...
            if (hasUrl(imageryProvider) && providerCoords[imageryProvider.url]) {
                ...
                promises.push(imageryProvider
                    .pickFeatures(coords.x, coords.y, coords.level, pickPosition.longitude, pickPosition.latitude)
            ...
            runInAction(() => {
                ...
                result.features = allFeatures.reduce((resultFeaturesSoFar, imageryLayerFeatures, _i) => {
                    ...
                    if (this.terria.showSplitter &&
                        ...
                        const pickedSide = screenPosition &&
    ...
    _computePositionOnScreen(position, result) {
        return SceneTransforms.worldToWindowCoordinates(this.scene, position, result);
    }
    _selectFeature() {
        const feature = this.terria.selectedFeature;
...
__decorate([
    computed
], Cesium.prototype, "availablePrimitives", null);
__decorate([
    computed
...
function syncImageryCollection(imageryCollection, imageryLayers, isOwner) {
